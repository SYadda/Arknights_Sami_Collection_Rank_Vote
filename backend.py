...
lst_effect = Config.LIST_EFFECT
lst_price = Config.LIST_PRICE
lst_name = list(dict_name.keys())
len_lst_name_1 = len(lst_name) - 1
set_code = set()


@app.route('/new_compare', methods=['GET']) #6星投票箱的new_compare删掉了，6星投票箱的new_operator_compare重命名为new_compare
@cross_origin()
def new_compare():
    ...
    return lst_name[a] + ' ' + lst_name[b] + ' ' + lst_effect[a] + ' ' + lst_effect[b] + ' ' + lst_price[a] + ' ' + lst_price[b] + ' ' + str(code_random)
